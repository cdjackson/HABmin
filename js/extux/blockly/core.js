...
Blockly.BlockSvg.prototype.connectionUiEffect = function () {
    ...
    var ripple = Blockly.createSvgElement('circle',
        {'cx': xy.x, 'cy': xy.y, 'r': 0, 'fill': 'none', 'stroke': '#888', 'stroke-width': 10},
        Blockly.svg);
    // Start the animation.
    ...
    for (var y = 0, row; row = inputRows[y]; y++) {
        ...
        if (this.block_.isCollapsed()) {
            ...
            for (var x = 0, input; input = row[x]; x++) {
                ...
                if (input.type == Blockly.INPUT_VALUE) {
                    ...
                    if (Blockly.RTL) {
                        ...
                        highlightInlineSteps.push('v', input.renderHeight - Blockly.BlockSvg.TAB_HEIGHT + 3);
                        highlightInlineSteps.push('h', input.renderWidth - Blockly.BlockSvg.TAB_WIDTH + 1);
                    } else {
            ...
            if (Blockly.RTL) {
                ...
                highlightSteps.push('M',
                        (cursorX - Blockly.BlockSvg.NOTCH_WIDTH + Blockly.BlockSvg.DISTANCE_45_OUTSIDE) + ',' +
...
Blockly.FieldLabel.prototype.dispose = function () {
    Ext.removeNode(this.textElement_);
    this.textElement_ = null;
};
...
Blockly.Mutator = function (quarkNames) {
    ...
    for (var x = 0; x < quarkNames.length; x++) {
        var element = Ext.DomHelper.createDom({tag: 'block', 'type': quarkNames[x]});
...
Blockly.Tooltip.onMouseOver_ = function (e) {
    ...
    while (!Ext.isString(element.tooltip) && !Ext.isFunction(element.tooltip)) {
